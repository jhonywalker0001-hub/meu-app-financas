...
class Dashboard(MDBoxLayout):
    ...
    def abrir_seletor_mes(self, *args):
        # Implementação para Android: salva com nome padrão na pasta do App
        try:
            mes_atual = datetime.now().strftime("%m_%Y")
            nome_arquivo = f"Relatorio_{mes_atual}.xlsx"
            exportar_para_excel(nome_arquivo, "Geral")
            Snackbar(text=f"Relatório exportado: {nome_arquivo}").open()
    ...
    def abrir_dialogo_edicao(self, trans):
        ...
        self.edit_valor = MDTextField(text=str(trans['valor']), hint_text="Valor", input_filter="float")
        ...
        self.dialog_edit = MDDialog(
            ...
            buttons=[
                ...
                MDFlatButton(text="SALVAR", theme_text_color="Custom", text_color=(0, 0.8, 0.4, 1),
                             on_release=lambda x: self.salvar_edicao(trans['id'], trans['tipo']))
            ]
        )
        self.dialog_edit.open()

    def salvar_edicao(self, id_t, tipo_t):
        try:
            novo_valor = float(self.edit_valor.text)
